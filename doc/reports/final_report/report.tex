...
For implementation of a recurrent neural network Python \citep{python} and the library Theano \citep{theano} with extension of Climin \citep{climin} for the networks parameter weight initialization by random uniform distribution was used. Furthermore implementations of various RNNs is provided by Breze \citep{breze} and used to extend the initial from scratch attempt.

The network is design having a single hidden recurrent layer of tangent-hyperbolicus activation function neurons. At the output layer a sigmoid activation function is chosen. Evaluating several optimizers, Adadelta showed most promising results, configured having $0.9$ memory decay and $0.9$ momentum, $1e-6$ offset and a step rate of $0.1$. This is constant over all combinations and subsets of data, EEG just the same as EMG. RmsProp with the same configuration as Adadelta but a smaller step rate of $1e-4$ did reach most closely but couldn't meet Adadeltas performance. At the output layer concerning the one-hot-encoded targets the Bernoulli-cross-entropy loss is used.
...
\subsection{Targets}
The targets for the RNN are defined by specific time events given in the dataset. For the experiments of this paper, the following five targets were used:
\begin{enumerate}
...
\section{Results}
All our RNNs were trained to predict the previously defined targets using EMG or EEG data. In this chapter, at first the results with EMG data and then the results using EEG data are presented.
